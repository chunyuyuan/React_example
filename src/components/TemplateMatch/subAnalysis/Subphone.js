...
export default class Subphone extends Component {
  constructor(props) {
    super(props);
  ...
  state ={
        ...
        async componentDidUpdate(prevProps, prevState) {
          if (prevProps.valuemin !== this.props.valuemin||prevProps.valuemax !== this.props.valuemax) {
            this.setState({valuemin:this.props.valuemin,valuemax:this.props.valuemax},()=>{
              axios.get('http://127.0.0.1:5000/phoneanalysis1?valuemin=$'+this.state.valuemin+'&valuemax=$'+this.state.valuemax).then (res=>{
                // console.log(res);
        ...
        axios.get('http://127.0.0.1:5000/phoneanalysis?valuemin=$'+this.state.valuemin+'&valuemax=$'+this.state.valuemax).then (res=>{
           // console.log(res);
            var x=res.data.split("&&");
          // console.log(JSON.parse(x[0]) )
            this.setState({template:JSON.parse(x[0]),graph1:JSON.parse(x[1]),graph2:JSON.parse(x[2]),graph3:JSON.parse(x[3]),graph4:JSON.parse(x[4]),graph5:JSON.parse(x[5])});
     ...
     const style = {
      ...
      background: "#f0f0f0"
        ...
        return (
          ...
          <div  style={{backgroundColor: "lightblue",fontSize:"10px"}}>   {"Phone analysis"}</div>
        { /*    <VictoryChart 
height={this.state.height} width={this.state.width} 
 ...
 >
      ...
      data: { stroke: "gold" }
                ...
                <VictoryAxis dependentAxis style={{ tickLabels: { fontSize: 20 } ,axisLabel: {fontSize:  25, padding: 30}}} domain={{y:[0,]}}   tickFormat={(t) => `${Math.round(t)}`} label="phone call freq"/>
                 <VictoryAxis  offsetY={50} style={{ tickLabels: { fontSize: 30 } ,axisLabel: {fontSize: 30, padding: 30}}}domain={{x:[1,]}}  label="phone call freq node distribution"/>
 ...
 >
      ...
      data: { stroke: "gold" }
                 ...
                 <VictoryAxis dependentAxis style={{ tickLabels: { fontSize: 20 } ,axisLabel: {fontSize:  25, padding: 30}}} domain={{y:[0,]}}   tickFormat={(t) => `${Math.round(t)}`} label="phone receive freq"/>
                 <VictoryAxis  offsetY={50} style={{ tickLabels: { fontSize: 30 } ,axisLabel: {fontSize: 30, padding: 30}}}domain={{x:[1,]}}  label="phone receive freq node distribution"/>
 ...
 >
      ...
      data: { stroke: "gold" }
                 ...
                 <VictoryAxis dependentAxis style={{ tickLabels: { fontSize: 20 } ,axisLabel: {fontSize: 25, padding: 30}}} domain={{y:[-2,]}}   tickFormat={(t) => `${Math.round(t)}`} label="phone  freq"/>
                 <VictoryAxis  offsetY={50} style={{ tickLabels: { fontSize: 30 } ,axisLabel: {fontSize: 30, padding: 30}}} label="phone communication freq distribution"/>
