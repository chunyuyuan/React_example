...
import Graphdeci from './Graphdeci'
export default class Graphattr extends Component {
    constructor(props) {
        super(props);
      
       this. state ={
        x: 713 ,y:152,width:412,height:180,
          g1:false,
          demog:false,
          travel:false,
          item: false,
    ...
    handleZoom(domain) {
      ...
      activateLasers=()=>{
        this.setState({x: 713 ,y:152,width:412,height:180});
      }
      componentDidMount(){
    ...
    this.getJoke();
      ...
      getJoke() {
        ...
        axios.get('http://127.0.0.1:5000/allanalysis1?valuemin=$'+this.props.timemin+'&valuemax=$'+this.props.timemax+'&rate='+JSON.stringify("")).then (res=>{
          var x=res.data.split("&&"); 
          console.log(x)           
         
          this.setState({value:JSON.parse(x[0]),valuerate:JSON.parse(x[1]) },()=>{console.log("hello"+x[1])}) // console.log(this.state.gr)
    ...
    render() {
let part;
part=<Graphdeci rate={this.state.valuerate}/>
        const arr=["phone call", "phone receive","phone communication","email send","email receive","email communication","procurement freq","procurement weight","demographics freq","demographics weight","source location","target location","trip time"]
  
console.log(this.props.timemax)
        const rendercancel = ()=> {
   ...
   cancel ={ rendercancel()}
 >
       <div  style={{backgroundColor: "lightblue",fontSize:"10px" }}>  {"Graph Node Attribute Difference Normalization Analysis"} <button style={{position: 'absolute',
 ...
 >
    <VictoryLine data={this.state.value}  y="graph1" x="index" style={{
      data: { stroke: "red" }   
      
     
    }}/>
   <VictoryScatter data={this.state.value}
    y="graph1" x="index"
          ...
          /> 
     <VictoryLine data={this.state.value}  y="graph2" x="index" style={{
    ...
    y="graph2" x="index"
          ...
          /> 
     <VictoryLine data={this.state.value}  y="graph3" x="index" style={{
      data: { stroke: "green" }   
    ...
    y="graph3" x="index"
          ...
          /> 
     <VictoryLine data={this.state.value}  y="graph4" x="index" style={{
      data: { stroke: "brown" }   
   ...
   <VictoryScatter data={this.state.value}
    y="graph4" x="index"
          ...
          /> 
         <VictoryLine data={this.state.value}  y="graph5" x="index" style={{
      data: { stroke: "gold" }   
