...
def graph_analysis(): 
   ...
   # print(selectvalue,checked)
    a=compute.drawfig("./data/CGCS-Templaterew.csv","template",minv,maxv,book,item,travel,demog,selectvalue,checked)  
    b=compute.drawfig("./data/Q1-Graph1rew.csv","graph1",minv,maxv,book,item,travel,demog,selectvalue,checked)
    c=compute.drawfig("./data/Q1-Graph2rew.csv","graph2",minv,maxv,book,item,travel,demog,selectvalue,checked)  
    d=compute.drawfig("./data/Q1-Graph3rew.csv","graph3",minv,maxv,book,item,travel,demog,selectvalue,checked) 
    e=compute.drawfig("./data/Q1-Graph4rew.csv","graph4",minv,maxv,book,item,travel,demog,selectvalue,checked) 
    g=compute.drawfig("./data/Q1-Graph5rew.csv","graph5",minv,maxv,book,item,travel,demog,selectvalue,checked)
 #   p=Pool(6)

  #  a1=p.apply_async(compute.drawfig,args=("./data/CGCS-Templaterew.csv","template",minv,maxv,book,item,travel,demog,selectvalue,checked))
   # b1=p.apply_async(compute.drawfig,args=("./data/Q1-Graph1rew.csv","graph1",minv,maxv,book,item,travel,demog,selectvalue,checked))
    #c1=p.apply_async(compute.drawfig,args=("./data/Q1-Graph2rew.csv","graph2",minv,maxv,book,item,travel,demog,selectvalue,checked))
    #d1=p.apply_async(compute.drawfig,args=("./data/Q1-Graph3rew.csv","graph3",minv,maxv,book,item,travel,demog,selectvalue,checked))
   # e1=p.apply_async(compute.drawfig,args=("./data/Q1-Graph4rew.csv","graph4",minv,maxv,book,item,travel,demog,selectvalue,checked))
    #g1=p.apply_async(compute.drawfig,args=("./data/Q1-Graph5rew.csv","graph5",minv,maxv,book,item,travel,demog,selectvalue,checked))
    #p.close()
    #p.join()    
  ...
  #  valuea.append(valuec)
   ...
   # print(valuea)
    ...
    return str(a)+"&&"  +str(b) +"&&"  +str(c)+"&&"  +str(d)+"&&"  +str(e) +"&&"  +str(g)
...
def graph_analysis1(): 
  ...
  #  valuea["index"]=["Degree Centrality","Closeness Centrality","Betweenness Centrality","Katz Centrality","Harmonic Centrality","Pagerank","Number of Nodes","Number of Edges","Degree","Average Neighbor Degree"]
    ...
    return normalized_df.to_json(orient='records')+"&&"+rerate.to_json(orient='records')
@app.route("/allanalysis1")
def allanalysis(): 
    print(int( request.args.get('valuemin')[1:]))
    minv =int( request.args.get('valuemin')[1:])
    maxv = int(request.args.get('valuemax')[1:])  
    oldrate=(request.args.get('rate'))
    
    normalized_df= compute.analysis(minv,maxv)
    ...
    print(vrate)
    rerate=[vg1+vrate[0],vg2+vrate[1],vg3+vrate[2],vg4+vrate[3],vg5+vrate[4]]
    print(rerate)
    rerate=pd.DataFrame(rerate)
    rerate.columns=['value']
    rerate['index']=['graph1','graph2','graph3','graph4','graph5']
